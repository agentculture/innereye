...
def _backend(args: argparse.Namespace) -> ComfyUIBackend:
    return ComfyUIBackend(
        getattr(args, "endpoint", DEFAULT_ENDPOINT),
        timeout=getattr(args, "timeout", _http.DEFAULT_TIMEOUT),
    )
...
def collect(
    ...
    *,
    ...
    for filename, data in artifacts:
        target = out_dir / f"{record.id}_{filename}"
        provenance = prov.Provenance(
            ...
            width=record.recipe.get("params", {}).get("width"),
            height=record.recipe.get("params", {}).get("height"),
            steps=record.recipe.get("params", {}).get("steps"),
            sampler=str(record.recipe.get("params", {}).get("sampler", "")),
            fps=record.recipe.get("params", {}).get("fps"),
            length=record.recipe.get("params", {}).get("length"),
        )
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    state = _refresh(_backend(args), record)
    payload = {
        "job": record.id,
        "backend_job_id": record.backend_job_id,
        "state": state["state"],
...
def register(sub: argparse._SubParsersAction) -> None:
    ...
    st.add_argument("job_id", help="Job id from 'innereye render --apply'.")
    _add_common(st)
    st.set_defaults(func=cmd_status)

    fe = noun_sub.add_parser("fetch", help="Download a finished job's artifacts.")
    fe.add_argument("job_id", help="Job id from 'innereye render --apply'.")
    ...
    ca.add_argument("job_id", help="Job id from 'innereye render --apply'.")
    _add_common(ca)
