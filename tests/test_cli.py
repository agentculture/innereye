"""Smoke tests for the innereye CLI entry point and its verbs."""
...
from innereye.explain import known_paths
...
def test_no_args_prints_help(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main([])
    assert rc == 0
    assert "usage: innereye" in capsys.readouterr().out
...
def test_explain_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "innereye whoami" in payload["markdown"]
