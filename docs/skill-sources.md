...
innereye vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. innereye
...
#   - consumer-identifying prose: `guildmaster` → `innereye` (NOT
...
If a re-sync would lose a innereye adaptation, lift the change
...
#  `which innereye vendors from guildmaster`).
