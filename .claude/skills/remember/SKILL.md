...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope innereye
...
scope:** the default personal scope is **private** (`--scope innereye
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `innereye`). Private records are served only to a recall in the same
