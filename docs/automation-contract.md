...
harness in a clone of `innereye`, and is the human-readable
...
```sh
claude -p "Answer with exactly one word, yes or no: does CLAUDE.md describe this project as innereye?"
```
...
```sh
qwen --approval-mode plan --output-format text "Answer with exactly one word, yes or no: does QWEN.md describe this project as innereye?"
```
...
|---|---|---|
| `claude -p "…innereye?"` | `Yes` | `?? docs/harness-invocations.yaml` (pre-existing; otherwise empty) |
| `pi -p --approve --no-tools "…lobes-cli?"` | `yes` | `?? docs/harness-invocations.yaml` (pre-existing; otherwise empty) |
| `qwen --approval-mode plan --output-format text "…innereye?"` | `yes` | `?? docs/harness-invocations.yaml` (pre-existing; otherwise empty) |
